...
const registerManual = asyncHandler(async (req, res) => {
  const invoice = await PaymentService.registerManual(parseInt(req.params.invoiceId || req.params.id, 10), req.body, req.user, req.ip);
...
const initiateExternal = asyncHandler(async (req, res) => {
  const transaction = await PaymentService.initiateExternal(parseInt(req.params.invoiceId || req.params.id, 10), req.body, req.user, req.ip);
...
const refreshExternalStatus = asyncHandler(async (req, res) => {
  const transaction = await PaymentService.refreshExternalStatus(parseInt(req.params.id, 10), req.user, req.ip);
