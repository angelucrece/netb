...
const getById = asyncHandler(async (req, res) => {
  const site = await SiteService.getSiteById(Number.parseInt(req.params.id));
  success(res, site);
});
...
const update = asyncHandler(async (req, res) => {
  const site = await SiteService.updateSite(Number.parseInt(req.params.id), req.body);
  success(res, site, 'Site mis à jour');
});

const toggle = asyncHandler(async (req, res) => {
  const site = await SiteService.toggleSite(Number.parseInt(req.params.id), req.body.active);
  success(res, site, `Site ${req.body.active ? 'activé' : 'désactivé'}`);
});

const remove = asyncHandler(async (req, res) => {
  await SiteService.deleteSite(Number.parseInt(req.params.id));
